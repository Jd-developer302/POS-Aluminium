...
export default function Index({ taxes }) {
    ...
    const canCreate =
        perms.includes('taxes.create') || perms.includes('settings.taxes');
    const canEdit =
        perms.includes('taxes.edit') || perms.includes('settings.taxes');
    const canDelete =
        perms.includes('taxes.delete') || perms.includes('settings.taxes');
    const canView =
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                            VAT / fixed charges — slug from name, unique code
                        </p>
                    </div>
                    {canCreate && (
                        <Link
                                            ...
                                            <div className="inline-flex items-center justify-end gap-1.5">
                                                ...
                                                {canDelete && (
                                                    <button
                                                        type="button"
                                                        title="Delete"
                                                        aria-label={`Delete tax ${t.name}`}
                                                        onClick={() =>
                                                            destroy(t.slug, t.name)
                                                        }
